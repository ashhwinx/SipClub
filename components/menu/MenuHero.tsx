...
import { Star, Plus, Ticket } from 'lucide-react';
...
const MenuHero: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      // 1. Hero Text Parallax (Moving on Scroll - Optimized)
      gsap.to(heroText1Ref.current, {
        xPercent: -15, // Moves left on scroll
        force3D: true,
        scrollTrigger: {
      ...
      gsap.to(heroText2Ref.current, {
        xPercent: 15, // Moves right on scroll
        force3D: true,
        scrollTrigger: {
          trigger: containerRef.current,
          start: 'top top',
      ...
      // 3. Sticker Rotation (Slow & Constant)
      gsap.to(".est-sticker", {
        rotation: 360,
        repeat: -1,
        duration: 45, // Very slow rotation
        ease: "none"
      });
    }, containerRef);
  ...
  return (
    <section ref={containerRef} className="relative flex h-[100vh] w-full flex-col items-center justify-center overflow-hidden bg-[#FEF8DD] pt-20 md:pt-24 border-b-4 border-[#004632]">
      ...
      {/* --- FLOATING BLURRED SHAPES (Optimized) --- */}
      <div ref={shapesRef} className="absolute inset-0 pointer-events-none z-0 overflow-hidden transform-gpu will-change-transform">
      ...
      {/* Top Right Ticket */}
      <div className="absolute top-20 right-4 md:top-32 md:right-16 z-20 scale-75 md:scale-100 origin-top-right transform-gpu">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-heading text-[25vw] text-[#BDD0A0] opacity-30 pointer-events-none select-none z-0 mix-blend-multiply transform-gpu">
          メニュー
      </div>

      {/* --- MAIN HERO TEXT (With Parallax) --- */}
      <div className="z-10 flex flex-col items-center leading-[0.85] select-none relative mix-blend-darken w-full px-4 transform-gpu">
          
          {/* Text 1: TASTE (Moves Left on scroll) */}
          <h1 ref={heroText1Ref} className="font-heading text-[15vw] md:text-[12vw] text-[#004632] tracking-tighter whitespace-nowrap will-change-transform">
              TASTE
          </h1>
          ...
          <div className="flex items-center gap-4 w-full max-w-xs md:max-w-2xl justify-center my-2 md:my-4">
              ...
              <Star size={20} fill="#004632" className="text-[#004632] " />
              <div className="h-1 md:h-2 flex-grow bg-[#004632]"></div>
          </div>
          
          {/* Text 2: THE HYPE (Moves Right on scroll) */}
          <h1 ref={heroText2Ref} className="font-heading text-[15vw] md:text-[12vw] text-[#004632] tracking-tighter whitespace-nowrap will-change-transform">
             THE HYPE
          </h1>
          
      </div>

      {/* --- ROTATING STICKER (Very Slow) --- */}
      <div className="est-sticker absolute bottom-8 right-4 md:bottom-12 md:right-12 z-20 flex h-20 w-20 md:h-28 md:w-28 items-center justify-center rounded-full bg-[#BDD0A0] p-1 text-[#004632] shadow-lg border-2 border-[#004632] transform-gpu will-change-transform">
