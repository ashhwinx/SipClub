...
import { UtensilsCrossed, Sticker, Ticket } from 'lucide-react';
...
const MenuSweetTooth: React.FC = () => {
  ...
  const handleEnter = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Smooth Hardware-Accelerated Zoom
    if (img) {
      gsap.to(img, { 
        scale: 1.1, 
        duration: 0.6, 
  ...
  return (
    <section className="w-full bg-[#FEF8DD] py-20 border-t-2 border-[#004632] overflow-hidden">
      <div className="container mx-auto px-4 md:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {/* LEFT: THE HERO IMAGE */}
            <div 
               className="col-span-1 lg:col-span-7 relative group cursor-pointer transform-gpu"
               onMouseEnter={handleEnter}
               onMouseLeave={handleLeave}
            >
             
               
               {/* Piche wala green part (Shadow & Rotate) ab static hai, hover par change nahi hoga */}
            ...
            <div className="col-span-1 lg:col-span-5 relative mt-8 lg:mt-0 transform-gpu">
               <div className="w-full bg-white border-2 border-[#004632] p-6 shadow-[8px_8px_0px_#BDD0A0] relative overflow-hidden">
                  ...
                  <div className="text-center border-b-2 border-dashed border-[#004632] pb-6 mb-6">
                     ...
                     <div className="font-mono text-xs text-[#004632] uppercase tracking-widest opacity-60">Tokyo Sip Club • Patisserie</div>
                  </div>
                        ...
                        <div key={item.id} className="group/item flex justify-between items-start cursor-default">
                           <div className="flex-1">
                              <div className="flex items-center gap-2">
                                 <h3 className="font-heading text-xl md:text-2xl text-[#004632] group-hover/item:underline decoration-wavy decoration-[#BDD0A0] transition-colors">
                                    {item.name}
                                 </h3>
                              ...
                              <p className="font-mono text-xs text-[#004632] opacity-60 mt-1 uppercase">{item.desc}</p>
                           </div>
                  ...
                  {/* Receipt Footer */}
                  <div className="mt-8 pt-6 border-t-2 border-dashed border-[#004632] text-center font-mono text-xs text-[#004632] opacity-70 uppercase">
                     <p>THANK YOU FOR VISITING</p>
                     <div className="flex justify-center mt-3">
                        <div className="h-8 w-48 bg-white border border-black/10 flex gap-[2px] px-1 items-center overflow-hidden">
                           {[...Array(30)].map((_, i) => (
                              <div key={i} className="h-full bg-black/80" style={{ width: (i % 2 === 0 ? 3 : 1) + 'px' }}></div>
                           ))}
                        </div>
                     </div>
                  </div>
